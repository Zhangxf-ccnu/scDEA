...
#' @param is.UMI logical. A logical flag to determine whether or not the input dataset is UMI-counts
...
#' @export
data_process <- function(Data, group, norm.form = "CPM", is.UMI = FALSE, is.normalized = FALSE){
  options(warn = -1)
  if(is.UMI){
    cat("The package need form use ERCC+number named spike-ins. \n")
    ...
    sce <- SingleCellExperiment::SingleCellExperiment(assays = list(counts = Data),
                                                      colData = cell_df,
                                                      rowData = gene_df)
    SingleCellExperiment::isSpike(sce, "ERCC") <- grepl("ERCC", rownames(Data))
    endog_genes <- as.matrix(SummarizedExperiment::assay(sce,"counts"))[!isSpike(sce),]
    spike.ins <- as.matrix(SummarizedExperiment::assay(sce,"counts"))[isSpike(sce),]
    normcounts <- normalized(endog_genes, method = norm.form)
    normcounts <- rbind(normcounts, spike.ins)
    ...
    sce <- SingleCellExperiment::SingleCellExperiment(assays = list(counts = Data, normcounts = normcounts),
                                                      ...
                                                      rowData = gene_df)
    SingleCellExperiment::isSpike(sce, "ERCC") <- grepl("ERCC", rownames(Data))
    sce <- scater::calculateQCMetrics(sce, feature_controls = list(ERCC = isSpike(sce)))
  } else
  {
    if(is.normalized){
...
Execute_BPSC <- function(object, coef = 2, BPSC.parallel = TRUE,...){
  ERCC_judge <- grepl("ERCC", rownames(object))
  if(sum(ERCC_judge) == 0){
    cat("The expression matrix don't include spike-ins genes.\n")
    object_BPSC <- as.matrix(SummarizedExperiment::assay(object, "normcounts"))
  } else{
    object_BPSC <- as.matrix(SummarizedExperiment::assay(object, "normcounts"))[!isSpike(object),]
  }
...
Execute_DEsingle <- function(object, DEsingle.parallel = TRUE){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_DEsingle <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
...
Execute_DESeq2 <- function(object, DESeq2.test = "Wald", beta.Prior = TRUE, DESeq2.parallel = TRUE, DESeq2.fitType = "parametric",...){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_DESeq2 <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
...
Execute_MAST <- function(object, method_MAST = "bayesglm", MAST.parallel = TRUE, ...){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_MAST <- as.matrix(SummarizedExperiment::assay(object, "normcounts"))[! isSpike(object),]
  }
...
# perform monocle
Execute_monocle <- function(object, monocle.parallel = TRUE,...){
  options(warn = -1)
  ERCC_judge <- grepl("ERCC", rownames(object))
  if(sum(ERCC_judge) == 0){
    cat("The expression matrix don't include spike-ins genes.\n")
    ...
    objects_monocle <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
  ...
  if(monocle.parallel){
    result_monocle <- monocle::differentialGeneTest(object_monocle, fullModelFormulaStr = "~ condition", cores = (parallel::detectCores() - 1),...)
  } else{
    result_monocle <- monocle::differentialGeneTest(object_monocle, fullModelFormulaStr = "~ condition", cores = 1,...)
  }
...
Execute_scDD <- function(object, alpha1 = 0.01, mu0 = 0, s0 = 0.01, a0 = 0.01, b0 = 0.01, scDD.permutation = 0, ...){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_scDD <- as.matrix(SummarizedExperiment::assay(object, "normcounts"))[! isSpike(object),]
  }
  ### creat scDD input
...
Execute_Wilcoxon <- function(object){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    cpm <- as.matrix(SummarizedExperiment::assay(object, "normcounts"))[! isSpike(object),]
  }
...
Execute_edgeR <- function(object, Test = "QLFT"){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_edgeR <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
...
Execute_limma <- function(object, method.fit = "ls", limma.trend =TRUE, limma.robust = TRUE){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_Limma <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
...
Execute_Seurat <- function(object,  method.Seurat = "bimod"){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_Seurat <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
  tmp <- object$label
...
Execute_zingeR.edgeR <- function(object, maxit.EM = 200){
  ...
  if(sum(ERCC_judge) == 0){
    ...
    object_zingerR.edgeR <- as.matrix(SummarizedExperiment::assay(object, "counts"))[! isSpike(object),]
  }
...
#' @param monocle.parallel a boolean variable that defines whether to execute parallel computing for monocle method. Default is TRUE.
...
scDEA_individual_methods <- function(raw.count, cell.label,
                                     ...
                                     monocle.parallel = TRUE, monocle.normalize = "CPM",
  ...
  if(BPSC == TRUE){
    ...
    sce.BPSC <- data_process(Data = raw.count, group = cell.label, norm.form = BPSC.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(DEsingle == TRUE){
    ...
    sce.DEsingle <- data_process(Data = raw.count, group = cell.label, norm.form = DEsingle.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(DESeq2 == TRUE){
    ...
    sce.DESeq2 <- data_process(Data = raw.count, group = cell.label, norm.form = DESeq2.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(edgeR == TRUE){
    ...
    sce.edgeR <- data_process(Data = raw.count, group = cell.label, norm.form = edgeR.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(MAST == TRUE){
    ...
    sce.MAST <- data_process(Data = raw.count, group = cell.label, norm.form = MAST.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(monocle == TRUE){
    ...
    sce.monocle <- data_process(Data = raw.count, group = cell.label, norm.form = monocle.normalize, is.UMI = FALSE, is.normalized = is.normalized)
    results.monocle <- Execute_monocle(sce.monocle, monocle.parallel = monocle.parallel)
  ...
  if(scDD == TRUE){
    ...
    sce.scDD <- data_process(Data = raw.count, group = cell.label, norm.form = scDD.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(Ttest == TRUE){
    ...
    sce.Ttest <- data_process(Data = raw.count, group = cell.label, norm.form = Ttest.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(Wilcoxon == TRUE){
    ...
    sce.Wilcoxon <- data_process(Data = raw.count, group = cell.label, norm.form = Wilcoxon.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(limma == TRUE){
    ...
    sce.limma <- data_process(Data = raw.count, group = cell.label, norm.form = limma.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(Seurat == TRUE){
    ...
    sce.Seurat <- data_process(Data = raw.count, group = cell.label, norm.form = Seurat.normalize, is.UMI = FALSE, is.normalized = is.normalized)
  ...
  if(zingeR.edgeR == TRUE){
    ...
    sce.zingeR.edgeR <- data_process(Data = raw.count, group = cell.label, norm.form = zingeR.edgeR.normalize, is.UMI = FALSE, is.normalized = is.normalized)
...
lancaster.combination <- function(Pvals, weight = TRUE, trimmed = 0.2){
  ...
  if(weight){
    ...
    num_pval <- ncol(Pvals)
    weight.pval <- rep(1/num_pval, num_pval)
